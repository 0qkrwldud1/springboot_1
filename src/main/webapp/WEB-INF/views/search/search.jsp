...
<script src="//dapi.kakao.com/v2/maps/sdk.js?appkey=c413e299c996a33dc44f7abdf4ac4223&libraries=services"></script>
</head>
<body>
...
<div id="content">
	<div class="food_nav_1">
		<div class="filter">
			<span>${foodKind} 맛집 인기 검색 순위</span>
	...
	<div class="food_nav_2">
		...
		<script type="text/javascript">
			...
			mapOption = {
				center: new kakao.maps.LatLng(35.17971927960649, 129.07507834435137), // 지도의 중심좌표
			    level: 10 // 지도의 확대 레벨
			};  
			// 지도를 생성합니다    
...
// searchCountAjax -> resultCount = data;의 값으로 페이징
...
function searchResultAjax() {
	...
	$.ajax({
		...
		success : function (data) {
					...
					// onclick='location.href=\""+url+"\"'>"; -> 해당 foodcode 음식점으로 이동
						...
						cont += "<div>";
							...
							if (data[i].reviewScore > 0) {
								// reviewScore ->
								// Math.round -> 메소드 사용시 값을 소수점 첫째 자리 까지 반올림 해준다.
								// Math.round(data[i].reviewScore * 10)/10 -> 소수점 둘째자리까지 보여줌
								// reviewScore -> 별점
			...
			$(".result").html(cont);	
			// 'location.href=\""+url+"\"'
			// url -> root + "/food/read.go?foodCode=" + data[i].foodCode;
			},
		error: function (request, status, error) {
			var str = 'code: '+request.status+'\n';
			str += 'message: ' + request.responseText+'\n';
